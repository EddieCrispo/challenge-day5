...
export default function AccountSection() {
  const { user } = useAuth();
  const { accounts, loading, error, fetchAccounts, addAccount, deleteAccount } =
    useAccountStore();
  ...
  return (
    <>
      {loading ? (
        ...
        <div className="flex gap-4 overflow-x-auto mb-6 pb-4">
          <div
            className="w-[80px] flex-shrink-0 flex items-center justify-center bg-slate-100 dark:bg-gray-700 hover:bg-slate-200 dark:hover:bg-gray-600 transition cursor-pointer rounded-lg p-6 border border-slate-200 dark:border-gray-600 overflow-hidden"
          ...
          {accounts.map((account) => (
            <div
              key={account.id}
              className="min-w-[360px] flex-shrink-0 relative"
            >
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl w-full max-w-md">
            ...
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-500">
                Address: <span className="font-medium">{user.address}</span>
              </p>
              <p className="text-sm text-gray-500">
